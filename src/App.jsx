...
const ProtectedRoute = ({ children, allowedRoles }) => {
  ...
  if (allowedRoles && !allowedRoles.includes(user?.role)) {
    ...
    if (role === 'Doctor' || role === 'Staff') return <Navigate to="/doctor-dashboard" replace />;
        ...
        <main className={`flex-1 transition-all duration-200 ${showSidebar ? 'lg:ml-64 p-4 sm:p-6 lg:p-8' : (isAuthenticated && isPatient ? 'p-4 sm:p-6 lg:p-8' : '')}`}>
          <ErrorBoundary>
            <Routes>
              ...
              {/* Hospital & Doctor / Clinical Staff Portal */}
              <Route path="/dashboard" element={<ProtectedRoute allowedRoles={['Hospital Admin']}><HospitalDashboard /></ProtectedRoute>} />
              <Route path="/doctor-dashboard" element={<ProtectedRoute allowedRoles={['Doctor', 'Staff']}><DoctorDashboard /></ProtectedRoute>} />
              <Route path="/patient-search" element={<ProtectedRoute allowedRoles={['Hospital Admin', 'Doctor', 'Staff', 'Super Admin']}><PatientSearchPage /></ProtectedRoute>} />
              <Route path="/create-patient" element={<ProtectedRoute allowedRoles={['Hospital Admin', 'Doctor', 'Staff', 'Super Admin']}><CreatePatientPage /></ProtectedRoute>} />
              <Route path="/clinical-snapshot" element={<ProtectedRoute allowedRoles={['Hospital Admin', 'Doctor', 'Staff']}><ClinicalSnapshotPage /></ProtectedRoute>} />
              <Route path="/medical-history" element={<ProtectedRoute allowedRoles={['Hospital Admin', 'Doctor', 'Staff']}><CompleteMedicalHistoryPage /></ProtectedRoute>} />
              <Route path="/add-record" element={<ProtectedRoute allowedRoles={['Hospital Admin', 'Doctor', 'Staff']}><AddMedicalRecordPage /></ProtectedRoute>} />
              <Route path="/upload-document" element={<ProtectedRoute allowedRoles={['Hospital Admin', 'Doctor', 'Staff']}><UploadMedicalDocumentPage /></ProtectedRoute>} />
              <Route path="/missing-records" element={<ProtectedRoute allowedRoles={['Hospital Admin', 'Doctor', 'Staff']}><MissingRecordsPage /></ProtectedRoute>} />
              <Route path="/doctors" element={<ProtectedRoute allowedRoles={['Hospital Admin', 'Doctor', 'Staff', 'Super Admin']}><DoctorsPage /></ProtectedRoute>} />
              <Route path="/hospital-profile" element={<ProtectedRoute allowedRoles={['Hospital Admin', 'Doctor', 'Staff', 'Government Admin', 'Super Admin']}><HospitalProfilePage /></ProtectedRoute>} />
